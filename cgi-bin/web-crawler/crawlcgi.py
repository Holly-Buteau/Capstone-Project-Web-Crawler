...
form = cgi.FieldStorage()# Get parameters in form submission
keyword = form.getvalue('stop_words')
searchType = form.getvalue('search_type')
startingUrl = form.getvalue('start_url')
userId = form.getvalue('user_id')# Set general crawler parameters
db_config = {
...
user_id = userId# Get unique number
for search_code
import time
...
search_options = {
    'search_code': '%s-%s' % (user_id, millis),
    #Generate search_code as composite user and random millisecond time,
    needed to detect user in DB 'search_type': searchType,
    #or 'BFS'
...
import json # Create placeholder
for links
resulting_links = []# Loop over search results and output each individual 'Link'
object field
for link in search_result:
    resulting_links.append({
        'url': link.url,
        'domain': link.domain,
...
except Exception: #No need to throw error here, the crawler generated an error much earlier, so to the browser anything else than a simple 'success' message is a 'stack error '
pass
